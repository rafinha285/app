...
router.get("/g/eps",async(req,res)=>{
  ...
  try{
    ...
    var result = await req.db.execute("SELECT id, animeid, seasonid, name, duration, resolution FROM episodes WHERE date_added >= ? LIMIT ? ALLOW FILTERING",[semana,count],{prepare:true})
    Console.log(result)
    await sleep(2)
